...
<li>
  ...
  <?php $sort_content4 = strip_tags($fullsearch->content);
    if (stripos($sort_content4, 'strategy') !== false) {
    //$arr_content = explode('strategy',$sort_content4);
      $arr_content = preg_split("/strategy/i", $sort_content4);

    if(strlen($arr_content[0])>30){
      echo $arr_content[0].'strategy'.$arr_content[1];
      }
    }
   // else
